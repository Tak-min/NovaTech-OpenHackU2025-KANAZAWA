    ...
    try {
        ...
        for (const station of STATION) {
            const distanceQuery = `
              ...
              `;
              const distanceResult = await pool.query(distanceQuery, [longitude, latitude, station.longitude, station.latitude]);
              const distance = distanceResult.rows[0].distance;

              if (distance <= STATION_RADIUS_METERS){
                ...
                const updateUserQuery = `
                  ...
                  RETURNING *;
                `;
                const updateUser = await pool.query(updateUserQuery, [userId]);
